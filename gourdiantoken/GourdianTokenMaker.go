// Package gourdiantoken provides a JWT-based token management system for access and refresh tokens.
//
// Features:
// - Creation and verification of access and refresh tokens
// - Configurable token expiration and claims
// - Support for both symmetric key signing (HMAC) and asymmetric key signing (RSA, ECDSA)
// - Integration with UUID for token and session IDs
// - Customizable token claims (e.g., roles, permissions)
...
//   - error: Any error encountered during token creation
//
// Example:
//
//	response, err := maker.CreateAccessToken(ctx, userID, "john_doe", "admin", sessionID, []string{"read", "write"})
//	if err != nil {
...
// Example:
//
//	response, err := maker.CreateRefreshToken(ctx, userID, "john_doe", sessionID)
//	if err != nil {
...
//
// Parameters:
//   - tokenString: Signed access token
//
// Returns:
//   - *AccessTokenClaims: Decoded access token claims
...
//	if err != nil {
//	    log.Fatalf("Failed to verify access token: %v", err)
//	}
...
//
// Parameters:
//   - tokenString: Signed refresh token
//
// Returns:
//   - *RefreshTokenClaims: Decoded refresh token claims
//   - error: Any error encountered during verification
//
// Example:
//
...
//	if err != nil {
//	    log.Fatalf("Failed to verify refresh token: %v", err)
//	}
