...
// be returned by PostgresSchemaSQL() and applied by gourdiantoken's own
// test setup. NewPostgresTokenRepository itself never applies this —
// see PostgresSchemaSQL's own doc comment for why. The query code itself
// is generated by sqlc from the same directory (see internal/postgresdb)
// into its own internal package, isolating generated code from
// hand-written code.
//
//go:embed internal/postgresdb/schema.sql
var postgresSchemaSQL string

// PostgresSchemaSQL returns gourdiantoken's Postgres token-repository
// schema (plain CREATE TABLE/INDEX IF NOT EXISTS, no gourdiantoken-specific
// magic) as text, for the caller to apply through their own project's
// migration tool (golang-migrate, Flyway, a plain SQL file run in CI,
// whatever you already use) before constructing NewPostgresTokenRepository.
//
// gourdiantoken deliberately never applies its own schema at runtime: doing
// so requires the runtime connection's role to have CREATE on the target
...
// gourdiantoken and its schema changes. See docs/postgres.md for the full
// pattern.
...
//
// This never applies gourdiantoken's schema itself — see PostgresSchemaSQL.
// The 3 tables it queries (gourdiantoken_revoked_tokens,
// gourdiantoken_rotated_tokens, gourdiantoken_tenant_revocations) must
// already exist before this is called, applied through your own project's
// migration tool. Calling this against a database where they don't exist
// yet doesn't fail here — it fails on the first actual repository method
// call (MarkTokenRevoke, IsTokenRevoked, ...) with a plain Postgres
// "relation does not exist" error.
//
// Initialization Steps:
//  1. Ping the pool to validate connectivity
//  2. Return the initialized repository instance
//
// Parameters:
//   - ctx: Context for the connectivity check
...
//   - error: If pool is nil or the connectivity check fails
//
// Example:
//
