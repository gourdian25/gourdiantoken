...
Initiatives that touch multiple stages are tracked as standalone plan documents in `docs/plan/<topic>-plan.md`, checked into the repo so they survive across sessions (mirrors `ECOSYSTEM_PLAN.md` at the repo root, which used the same format for the now-complete pgx+sqlc/coverage initiative across the whole gourdian25 workspace). Check `docs/plan/` before assuming a half-finished-looking feature is abandoned rather than mid-stage — e.g. as of this writing, `docs/plan/multi-tenant-support-plan.md`'s Stage 1 ("Tenant claim foundation," see "Multi-tenancy" above) is done, but Stages 2-6 (interface consolidation, tenant-scoped bulk revocation via a revocation epoch, repository backend standardization incl. two real `MarkTokenRotatedAtomic` bugs on Postgres/MongoDB, and the doc/CHANGELOG/version-bump pass) are not started — each stage pauses for review before the next begins, per that file's own instruction.
