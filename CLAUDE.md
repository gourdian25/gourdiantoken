...
- `gourdiantoken.go` — core types and the only public engine: config (`GourdianTokenConfig`), claims (`AccessTokenClaims`, `RefreshTokenClaims`), the `TokenRepository` and `GourdianTokenMaker` interfaces, and `JWTMaker` (the concrete `GourdianTokenMaker` implementation covering create/verify/revoke/rotate, key parsing for all signing methods, and claim validation).
...
- Enabling rotation/revocation spins up background goroutines (`cleanupRotatedTokens`, `cleanupRevokedTokens`) tied to an internal `context.WithCancel`, running on `config.CleanupInterval`.
...
Tests are split by concern rather than mirroring source files 1:1: `token.creation_test.go`, `token.verification_test.go`, `token.revocation_test.go`, `token.rotate_test.go`, `revocation.rotation_test.go`, `concurrency_test.go`, `cryptographic_test.go` (algorithm/key handling), `config.validation_test.go`, `edge.case_test.go`, `integration_test.go`, `gourdiantoken.repository_test.go` (cross-backend repository contract tests), `gourdiantoken.factories_test.go` (factory constructors), `gourdiantoken.benchmark_test.go`. Shared setup helpers live in `token.test.helper.go` (test-time maker/repo factories) and `token.bench.helper.go` (benchmark equivalents) — extend these rather than duplicating setup in individual test files.
