...
`gourdiantoken` is a single-package Go library (`package gourdiantoken`, module `github.com/gourdian25/gourdiantoken/v2`) providing JWT access/refresh token creation, verification, revocation, and rotation, with pluggable storage backends. There is no `main` package other than the demo in `example/`.
